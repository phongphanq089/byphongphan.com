...
export function usePackageManager() {
  const [packageManager, setPackageManagerState] = useState<PackageManager>(
    () => {
      if (typeof window !== "undefined") {
        ...
        if (saved) {
          try {
            return JSON.parse(saved) as PackageManager
          } catch {
            return (saved as PackageManager) || "pnpm"
          }
        }
      }
      return "pnpm"
  ...
  const setPackageManager = (value: PackageManager) => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("packageManager", JSON.stringify(value))
    }
  }
...
export type CodeBlockCommandProps = {
   ...
   * @example
   * ```tsx
   * <CodeBlockCommand * command onCopySuccess="{({" packageManager, pnpm="pnpm add react"> {
   ...
   * @example
   * ```tsx
   * <CodeBlockCommand * onCopyError="{(error)" pnpm="pnpm add react"> {
    ...
    >
      ...
      >
        <ScrollArea>
          <TabsList
            className={cn(
              "mb-2 bg-transparent px-4 inset-ring-0 [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:text-muted-foreground"
            )}
          >
            <IconSwap>
            ...
            {tabsFiltered.map(([key]) => {
              return (
                <TabsTrigger
                  key={key}
                  className="rounded-sm p-0 px-2"
                  value={key}
                >
                  <span className="-translate-y-[2px]">{key}</span>
                </TabsTrigger>
              )
            })}
          </TabsList>
        ...
        {tabsFiltered.map(([key, value]) => {
          return (
            <TabsContent key={key} value={key}>
              <pre
                data-pm={key}
                className="group/tabs-content-pre overscroll-x-contain rounded-lg bg-background p-4 leading-6 not-data-[pm=prompt]:overflow-x-auto"
              >
                <code
                  data-slot="code-block"
                  data-language="bash"
                  className="text-sm/none text-muted-foreground group-data-[pm=prompt]/tabs-content-pre:whitespace-normal"
                >
 ...
 *
 * <CodeBlockCommand add button")} shadcn {...convertNpmCommand("npx/>
 * ```
 */
