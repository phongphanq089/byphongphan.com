      ...
      {tokens.map((token, index) => {
        ...
        return (
          <span
            ...
            className={cn(
              token.word &&
                "rounded-sm bg-primary/10 px-0.5 py-px ring-1 ring-primary/20 ring-inset"
            )}
            style={style}
          >
            {token.content}
  ...
  return (
    <CodeBlockHeaderContext.Provider value={true}>
      <div
        ...
        className={cn(
          "flex min-h-9 shrink-0 items-center gap-2 border-b border-border px-3 py-0 text-sm",
          className
        )}
        {...props}
      />
...
function CodeBlockCopyButton({
  ...
  }, [value, document_, onCopy, onCopyError])
    ...
    <div
      ...
      className={cn(
        ...
        /* The fade means "there is more below", so it belongs to the collapsed
           ...
           line of code. */
        !expanded && "bg-gradient-to-t from-card via-card/85 to-transparent",
        className
      )}
    >
      <Button
...
export { ansiToLines, parseUnifiedDiff } from "./code-block-highlight"
